    ...
    def show(app_name)
      ...
      if options.state
        ...
        when 'quota'
          ...
          table_from_gears('echo "$(du -sh 2>/dev/null | cut -f 1)"', groups, opts) do |gear, data, group|
            [gear['id'], group.cartridges.collect{ |c| c['name'] }.join(' '), (data.chomp rescue 'error'), human_size(group.quota)]
          end
        when 'ssh'
