...
module WizardHelper
  ...
  def stub_user(auth=user_auth)
    stub_api_request(:get, 'broker/rest/user', auth).to_return(simple_user('foo'))
  end
  def stub_create_default_key
    stub_api_request(:post, 'broker/rest/user/keys', user_auth).
      with(:body => hash_including({:name => 'default', :type => 'ssh-rsa'})).
      to_return({:status => 201, :body => {}.to_json})
  end
  def stub_update_key(name)
    stub_api_request(:put, "broker/rest/user/keys/#{name}", user_auth).
  ...
  def stub_no_keys
    stub_api_request(:get, 'broker/rest/user/keys', user_auth).to_return(no_keys)
  end
  def stub_mock_ssh_keys(name='test')
    stub_api_request(:get, 'broker/rest/user/keys', user_auth).
      to_return({
        :body => {
          :type => 'keys',
          :data => [
...
#              ]),
  ...
  def stub_no_domains
    stub_api_request(:get, 'broker/rest/domains', user_auth).to_return(no_domains)
  end
  def stub_one_domain(name)
    stub_api_request(:get, 'broker/rest/domains', user_auth).
      to_return({
        :body => {
          :type => 'domains',
  ...
  def stub_one_application(domain_name, name)
    stub_api_request(:get, "broker/rest/domains/#{domain_name}/applications", user_auth).
      to_return({
        :body => {
          :type => 'applications',
          :data => [{
  ...
  def stub_simple_carts
    stub_api_request(:get, 'broker/rest/cartridges', user_auth).to_return(simple_carts)
  end
